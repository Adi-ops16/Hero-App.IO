...
const FeaturedApp = ({ featuredApps }) => {
    const { id, title, image, description, downloads, ratingAvg } = featuredApps
    return (
        <Link to={`/appDetails/${id}`}>
            <div className='inter h-112 flex flex-col justify-between bg-white p-4 py-4 px-2 rounded-md space-y-3 hover:scale-105 duration-500 cursor-pointer'>
                ...
                <div className='flex items-center justify-between'>
                    <div className=" bg-[#F1F5E8] text-[#00D390] py-1 px-2 rounded-md">
                        <span className='flex items-center gap-1'><Download className='w-4' /> {downloads}M</span>
                    </div>
